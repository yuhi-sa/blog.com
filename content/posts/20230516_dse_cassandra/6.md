---
title: "DS201学習記録：Cassandraのnodetoolコマンドについて"
date: 2023-06-10T09:00:23+09:00
draft: false
description: "DS201コースの学習記録。nodetoolの代表的なコマンド（status、info、drainなど）とストレステストの実行方法を解説します。"
...
nodetoolはApache Cassandraの管理ツールの一部で、Cassandraクラスターの実行時操作や監視をサポートします。
...
statusコマンドは、クラスタ全体の情報、特に各ノードの状態やそれぞれのノードに関する情報を表示します。
...
- Status: ノードの状態を示します。Upはノードが起動していることを意味し、Downはノードが停止していることを意味します。
- State: ノードの状態を示します。Normalは通常の状態、Leavingはクラスタから離脱中、Joiningはクラスタに参加中、Movingはデータを移動中であることを意味します。
- Address: ノードのIPアドレス。
...
infoコマンドは、接続されたノードに関する情報を表示します。
...
describeclusterコマンドは、クラスタ全体のノード間で共通の設定や各ノードが使用している現在のスキーマバージョンを表示します。
...
- Cluster Information: クラスタの情報（クラスタ名、スニッチの設定、動的エンドポイントスニッチの有効化、パーティショナの情報）が表示されます。
- Schema versions: 各ノードが使用しているスキーマバージョンが表示されます。
- Stats for all nodes: 全ノードの状態統計情報（Live（稼働中）、Joining（参加中）、Moving（移動中）、Leaving（離脱中）、Unreachable（到達不能））が表示されます。
- Data Centers: データセンターの情報（データセンター名、ノード数、ダウンしているノード数）が表示されます。
- Database versions: データベースのバージョン情報が表示されます。
- Keyspaces: キースペースの情報（キースペース名とレプリケーション戦略）が表示されます。
...
- TRACE: 最も詳細なログメッセージを表示します。
- DEBUG: デバッグ情報を表示します。
- INFO: 一般的な情報メッセージを表示します。
- WARN: 警告メッセージを表示します。
...
settraceprobabilityコマンドの実行結果の値は、クエリが保存される割合を表す小数値です。この値は、0（0%）から1（100%）までの範囲で指定します。保存されたトレースは、system_tracesキースペースで確認できます。
...
drainコマンドは、ノード上での書き込みを停止し、すべてのデータをディスクにフラッシュするコマンドです。
通常、このコマンドは、Apache Cassandra™ノードを停止する前に実行されます。ノードの停止前に実行することで、書き込みが完了し、データがディスクに保存されます。
...
`cassandra-stress write n=50000 no-warmup -rate threads=1`コマンドを使って、Cassandraクラスタに対する書き込みストレステストを行います。このテストでは、50,000の書き込み操作が1つのスレッドで実行され、"keyspace1"という名前のキースペースに対して行われる。
nodetool flushコマンドで、メモリ内のデータをディスクに強制的に書き出します。
nodetool statusコマンドで、クラスタの現在の状態を確認します。
最後に、cqlshツールを用いて、キースペースとテーブルの情報を確認します。

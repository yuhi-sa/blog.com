...
draft: false
description: "バターワースフィルタとは何か？最大平坦特性の数学的導出から双一次変換によるデジタル化、SciPyを使ったローパス/ハイパス/バンドパスフィルタの設計・Python実装までを解説します。"
...
- [FIRフィルタとIIRフィルタの比較]({{< ref "/posts/20260226_fir_iir/1.md" >}}) - バターワース（IIR）とFIRフィルタの特性の違いを解説しています。
- [ウィーナーフィルタの理論とPython実装]({{< ref "/posts/20260226_wiener_filter/1.md" >}}) - SNRに基づく最適フィルタとバターワースの手動設計の対比を理解できます。
- [ノッチフィルタの設計とPython実装]({{< ref "/posts/20260228_notch_filter/1.md" >}}) - IIRフィルタの応用例として特定周波数除去を解説しています。
...
- [サビツキー・ゴーレイフィルタの理論とPython実装]({{< ref "/posts/20260310_savitzky_golay/1.md" >}}) - バターワースがIIRフィルタであるのに対し、FIR型の多項式フィッティングによる平滑化フィルタを解説しています。
- [適応フィルタ（LMS/RLS）の理論とPython実装]({{< ref "/posts/20260310_adaptive_filter/1.md" >}}) - バターワースが固定設計のフィルタであるのに対し、入力信号に応じてパラメータを自動調整する適応フィルタを解説しています。

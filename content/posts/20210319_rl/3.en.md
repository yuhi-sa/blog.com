...
$$ V(s*t) \leftarrow V(s_t) + \alpha (G_t - V(s_t)) $$
Here, $G_t = r*{t+1} + \gamma r*{t+2} + \gamma^2 r*{t+3} + \dots + \gamma^{T-t-1} r_T$ is the discounted return from time $t$ onward.
...
$$ V(s*t) \leftarrow V(s_t) + \alpha (r*{t+1} + \gamma V(s*{t+1}) - V(s_t)) $$
Here, $r*{t+1} + \gamma V(s\_{t+1})$ is the TD target.
...
- **SARSA**:
  $$ Q(s*t, a_t) \leftarrow Q(s_t, a_t) + \alpha (r*{t+1} + \gamma Q(s*{t+1}, a*{t+1}) - Q(s*t, a_t)) $$
  SARSA uses the action value of the action $a*{t+1}$ actually selected at the next state $s_{t+1}$, making it an **on-policy** method. This means the policy used for action selection during learning is the same as the policy obtained through learning.
...
- Takahiro Kubo, [Introduction to Reinforcement Learning with Python](https://www.amazon.co.jp/dp/4065142989?tag=tomatomato119-22), Shoeisha (2019)

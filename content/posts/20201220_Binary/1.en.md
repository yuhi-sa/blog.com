...
One of the most important applications of binary exponentiation is [RSA encryption]({{< ref "/posts/20260225_rsa/1.en.md" >}}). RSA requires modular exponentiation of large integers for both encryption and decryption.

$$c = m^e \bmod n \quad \text{(encryption)}$$
$$m = c^d \bmod n \quad \text{(decryption)}$$

Here, $e$ and $d$ are integers hundreds to thousands of bits long, making computation infeasible without binary exponentiation.

### Computational Complexity Comparison

| Method                | Complexity  | Multiplications for $e = 2^{16} + 1$ |
| --------------------- | ----------- | ------------------------------------ |
| Naive exponentiation  | $O(e)$      | 65,537                               |
| Binary exponentiation | $O(\log e)$ | 17                                   |

Python's built-in `pow(base, exp, mod)` function uses binary exponentiation internally and can be used directly in RSA implementations.

## Related Articles

- [RSA Encryption: Theory and Python Implementation]({{< ref "/posts/20260225_rsa/1.en.md" >}}) - Explains how modular exponentiation serves as the core operation in RSA encryption and decryption.
- [Elliptic Curve ElGamal Encryption: Principles and Simplified Python Implementation]({{< ref "/posts/20201223_ElGamal/1.en.md" >}}) - A public-key cryptosystem that applies modular exponentiation to scalar multiplication on elliptic curves.
- [Diffie-Hellman Key Exchange Protocol: Theory and Python Implementation]({{< ref "/posts/20230907_DH/1.en.md" >}}) - Covers the Diffie-Hellman key exchange protocol based on modular exponentiation.
- [Security Certification Comparison: CISSP vs Japan's Registered Information Security Specialist]({{< ref "/posts/20260223_security_certs/1.en.md" >}}) - Compares security certifications whose exam scope includes cryptographic technologies such as RSA.

...
draft: false
description: "サポートベクターマシン（SVM）とは？マージン最大化の数理、RBF・多項式カーネルによる非線形分類、scikit-learnでの実装からハイパーパラメータ（C, gamma）の調整方法まで解説します。"
...
- [アンサンブル学習の手法と比較]({{< ref "/posts/20260226_ensemble_learning/1.md" >}}) - ランダムフォレスト・勾配ブースティングなどの別の分類手法を解説しています。
- [Self-Attentionの仕組みとPython実装]({{< ref "/posts/20260228_self_attention/1.md" >}}) - カーネルトリックとAttentionの類似性を理解できます。
- [K-means法とGMM：クラスタリングの理論とPython実装]({{< ref "/posts/20260226_kmeans_gmm/1.md" >}}) - 教師なし学習との対比で分類と構造発見の違いを理解できます。

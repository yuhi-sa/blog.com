...
Excluding constant terms, the log-likelihood function is $\sum_{j=1}^k x_j \log \mu_j$.
The constraint is $\sum_{j=1}^k \mu_j - 1 = 0$.
...
Since $\sum_{j=1}^k x_j = m$ is the total number of trials:

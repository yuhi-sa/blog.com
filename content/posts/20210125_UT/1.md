...
description: "Unscented変換（U変換）とは？シグマポイントの生成方法から非線形変換後の平均・共分散の推定アルゴリズム、UKFへの応用までPython実装付きで解説します。"
...
- [Unscented Kalman Filter（UKF）の理論とPython実装]({{< ref "/posts/20260226_ukf/1.md" >}}) - この記事で解説したU変換をカルマンフィルタに組み込んだUKFの完全な理論と実装を解説しています。
- [カルマンスムーザ（RTS Smoother）の理論とPython実装]({{< ref "/posts/20260223_rts_smoother/1.md" >}}) - カルマンフィルタの推定を全時刻データで平滑化するRTSスムーザを解説しています。

---

## 関連ツール

- [CalcBox - 暮らしの計算ツール](https://yuhi-sa.github.io/calcbox/) - 統計計算、行列計算など61種類以上の計算ツール

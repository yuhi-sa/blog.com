...
draft: false
description: "Unscented Kalman Filter（UKF）とは？シグマポイントを用いたUnscented変換の仕組みから状態推定アルゴリズム、EKFとの精度比較、Pythonでの実装方法を解説します。"
...
- [拡張カルマンフィルタ（EKF）の理論とPython実装]({{< ref "/posts/20260224_ekf/1.md" >}}) - UKFの前身であるEKFの線形化アプローチを解説しています。
- [Cubature Kalman Filter（CKF）の理論とPython実装]({{< ref "/posts/20260215_CKF/1.md" >}}) - UKFの重み問題を解決するキュバチャカルマンフィルタを解説しています。
- [パーティクルフィルタのPython実装]({{< ref "/posts/20260223_particle_filter/1.md" >}}) - UKFより高い非線形性にはパーティクルフィルタが有効です。
- [カルマンスムーザ（RTS Smoother）の理論とPython実装]({{< ref "/posts/20260223_rts_smoother/1.md" >}}) - UKFベースのスムーザへの拡張の基盤となるRTSスムーザを解説しています。
- [Unscented変換の基礎]({{< ref "/posts/20210125_UT/1.md" >}}) - UKFの核となるUnscented変換の詳細な解説です。
- [ウィーナーフィルタの理論とPython実装]({{< ref "/posts/20260226_wiener_filter/1.md" >}}) - 定常過程における最適線形フィルタ。UKFとの位置づけの違いを理解できます。

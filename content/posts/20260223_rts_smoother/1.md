...
関連記事：[指数移動平均（EMA）フィルタの周波数特性]({{< ref "/posts/20220206_ema/1.md" >}})、[Cubature Kalman Filter（CKF）の理論とPython実装]({{< ref "/posts/20260215_CKF/1.md" >}})、[信号処理におけるフィルタリング手法の基礎]({{< ref "/posts/20200901_Filter/1.md" >}})
...
- [カルマンフィルタの理論とPython実装]({{< ref "/posts/20260224_kalman_filter/1.md" >}}) - RTSスムーザの基盤となるカルマンフィルタの理論と実装を解説しています。
- [拡張カルマンフィルタ（EKF）の理論とPython実装]({{< ref "/posts/20260224_ekf/1.md" >}}) - 非線形モデルに対応するEKFを解説しています。非線形スムーザの基盤にもなります。
- [Unscented Kalman Filter（UKF）の理論とPython実装]({{< ref "/posts/20260226_ukf/1.md" >}}) - UKFベースのスムーザへの拡張の基盤となるUKFを解説しています。
- [粒子フィルタのPython実装：リサンプリング手法の比較]({{< ref "/posts/20260223_particle_filter/1.md" >}}) - 粒子スムーザの基盤となる粒子フィルタを解説しています。
- [時系列データの異常検知：統計的手法からカルマンフィルタまで]({{< ref "/posts/20260228_timeseries_anomaly/1.md" >}}) - RTSスムーザはオフライン異常検知の前処理としても有用です。

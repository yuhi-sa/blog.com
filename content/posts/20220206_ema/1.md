...
| [移動平均]({{< ref "/posts/20260225_moving_average/1.md" >}})  | なだらか         | $O(N)$     | 1（窓幅$N$）          | 優秀               |
| [バターワース]({{< ref "/posts/20260226_butterworth/1.md" >}}) | 急峻（次数依存） | $O(N)$     | 2（次数、遮断周波数） | 良好               |
| [FIR/IIR]({{< ref "/posts/20260226_fir_iir/1.md" >}})          | 設計次第         | $O(N)$     | 多数                  | FIR:良好, IIR:優秀 |

EMAは1次のIIRフィルタとして見ることができ、パラメータ$\alpha$のみで制御できる点が実務上の大きな利点です。より急峻な遮断特性が必要な場合は[バターワースフィルタ]({{< ref "/posts/20260226_butterworth/1.md" >}})を、位相遅れを最小化したい場合は[ウィーナーフィルタ]({{< ref "/posts/20260226_wiener_filter/1.md" >}})を検討してください。

## scipy.signalを用いたEMAの実装

Pythonの`scipy.signal`モジュールを使うと、EMAをIIRフィルタとして実装できます。
...
この実装は自前のループ実装よりも高速で、`scipy.signal`の他のフィルタ関数（`butter`、`firwin`等）と統一的なインターフェースで扱えます。
...
- [窓関数とパワースペクトル密度（PSD）の理論とPython実装]({{< ref "/posts/20260228_fft_window_psd/1.md" >}}) - FFTにおける窓関数の数学的背景とPSD推定のWelch法を解説しています。
- [時系列データの異常検知：統計的手法からカルマンフィルタまで]({{< ref "/posts/20260228_timeseries_anomaly/1.md" >}}) - EMAを使った異常検知手法とカルマンフィルタベースの手法を比較しています。
- [ノッチフィルタの設計とPython実装]({{< ref "/posts/20260228_notch_filter/1.md" >}}) - EMAがローパス特性を持つのに対し、特定周波数のみを除去するノッチフィルタの設計を解説しています。
- [バターワースフィルタの設計とPython実装]({{< ref "/posts/20260226_butterworth/1.md" >}}) - EMAと異なり、通過帯域が最大限平坦な周波数特性を持つバターワースフィルタの設計を解説しています。
- [FIRフィルタとIIRフィルタの比較]({{< ref "/posts/20260226_fir_iir/1.md" >}}) - EMA（IIRフィルタの一種）の位置づけを、FIRフィルタとの比較から理解できます。
- [ウィーナーフィルタの理論とPython実装]({{< ref "/posts/20260226_wiener_filter/1.md" >}}) - 統計的最適性に基づくフィルタ設計。EMAとは異なるアプローチでノイズ除去を行います。
- [ウェーブレット変換の基礎とPython実装]({{< ref "/posts/20260226_wavelet/1.md" >}}) - 時間-周波数解析により、EMAでは対応しにくい非定常信号のフィルタリングを解説しています。
- [Unscented Kalman Filter（UKF）の理論とPython実装]({{< ref "/posts/20260226_ukf/1.md" >}}) - EMAの確率的拡張であるカルマンフィルタを、さらに非線形システムに対応させたUKFを解説しています。
- [サビツキー・ゴーレイフィルタの理論とPython実装]({{< ref "/posts/20260310_savitzky_golay/1.md" >}}) - EMAと異なり、多項式フィッティングにより信号の形状を保存しながら平滑化するフィルタを解説しています。
- [適応フィルタ（LMS/RLS）の理論とPython実装]({{< ref "/posts/20260310_adaptive_filter/1.md" >}}) - EMAが固定パラメータのフィルタであるのに対し、環境に適応してパラメータを自動調整する適応フィルタを解説しています。
